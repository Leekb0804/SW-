...
int ShortestDistance(int npcX, int npcY) {
	...
	Queue* q = (Queue*)calloc(((WIDTH * 2) * (HEIGHT * 2)), sizeof(Queue));
	Queue* safety = (Queue*)calloc(((WIDTH * 2) * (HEIGHT * 2)), sizeof(Queue));
	for (int i = 0; i < HEIGHT; i++) {
		for (int j = 0; j < WIDTH; j++) {
			one_srt_dist[i][j] = 0;
			weight[i][j] = 0;
	...
	bfs(q, safety, npcX, npcY); // 여기에 NPC의 출발(x,y)좌표를 집어넣는다.
